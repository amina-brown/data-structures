...
$("td[style='border-bottom:1px solid #e3e3e3; width:260px']").each(function(i, elem) {
    ...
    if ($(elem).html().split('<br>')[2].trim().split(',')[0] == "715 W 179TH ST"){
        floorInfo.push("1st Floor");
    ...
    if ($(elem).html().search("detailsBox") != -1){
        details.push($(elem).html().split('div')[1].split('\t')[1].split('\n')[0].replace("&apos;","'").replace("&amp;","&").replace("<br>","").replace("<br>No","No").trim());
    } else {
        details.push('');
    }
...
, function() {
    fs.writeFileSync('data/z10.json', JSON.stringify(meetingsData));
    // console.log('*** *** *** *** ***');
